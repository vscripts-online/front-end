...
const User = ({ table }: TableProp) => {
  const [id, setId] = useState<number>();
  const [open, setOpen] = useState(false);

  function handleFilter() {
    table.getColumn("user")?.setFilterValue(id);
    setOpen(false);
  }
    ...
    <div className="flex items-center gap-1">
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <div>
            <Trigger table={table} accessor="user" />
          </div>
        </PopoverTrigger>
...
export { CreatedAt, FileName, MimeType, Size, User, UpdatedAt };
